...
contract Vault is ReentrancyGuard, IVault {
    ...
    function tokenToUsd(address _token, uint256 _tokenAmount) public view returns (uint256) {
        ...
        uint256 price = tokenUtils.getPrice(_token);
        uint256 decimals = tokenUtils.getDecimal(_token);
    ...
    function increasePosition(
        ...
        feesUsd.add(positionFee);

        uint256 collateralDelta = _transferIn(_collateralToken);
        uint256 collateralDeltaUsd = tokenToUsd(_collateralToken, collateralDelta);
        position.collateral = position.collateral.add(collateralDeltaUsd);
        position.collateral = position.collateral.sub(feesUsd);

        // step 3: size
        position.size = position.size.add(_sizeDelta);

        // step 4: reserve
        uint256 reserveDelta = usdToToken(_collateralToken, _sizeDelta);
